...
<section class="py-16 bg-surface transition-colors duration-300">
    <div class="container mx-auto px-4">
        <h2 class="text-3xl font-bold text-text mb-10 text-center">Cities in <?= esc($location['name']) ?></h2>
        <?php if (empty($cities)): ?>
            <p class="text-center text-text-muted">City pages for <?= esc($location['name']) ?> are being prepared.</p>
        <?php else: ?>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <?php foreach($cities as $city): ?>
            <a href="<?= base_url('locations/' . $country['slug'] . '/' . $location['slug'] . '/' . $city['slug']) ?>" class="group block relative rounded-xl overflow-hidden shadow-md border border-border bg-surface hover:border-brand-primary/50 transition-all duration-300 p-5">
                <h3 class="text-lg font-bold text-text group-hover:text-brand-primary transition-colors"><?= esc($city['name']) ?></h3>
            </a>
            <?php endforeach; ?>
        </div>
        <?php endif; ?>
...
<!-- Relevant services for this region -->
<section class="py-16 transition-colors duration-300 border-t border-border">
    <div class="container mx-auto px-4">
        <h2 class="text-3xl font-bold text-text mb-3 text-center">Services for <?= esc($location['name']) ?> Businesses</h2>
        <p class="text-text-muted text-center mb-10 max-w-2xl mx-auto">Explore the capabilities we deliver for companies in this region.</p>
        <div class="flex flex-wrap justify-center gap-4">
            <?php foreach ($countryServices as $rs): ?>
            <a href="<?= base_url('services/' . esc($rs['slug'])) ?>" class="px-6 py-3 rounded-xl border border-border bg-surface text-text font-medium hover:border-primary hover:text-primary transition-colors">
                <?= esc($rs['name']) ?>
            </a>
            <?php endforeach; ?>
        </div>
...
<section class="py-16 bg-surface transition-colors duration-300 border-t border-border">
    <div class="container mx-auto px-4 text-center max-w-3xl">
        <h2 class="text-3xl font-bold text-text mb-4">Start a project in <?= esc($location['name']) ?></h2>
        <p class="text-text-muted mb-8">Share your goals with us and receive a tailored proposal.</p>
