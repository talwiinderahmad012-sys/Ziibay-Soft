...
<div class="px-6 py-8">
    <div class="flex items-center gap-3 mb-8">
        ...
        <h1 class="text-3xl font-bold text-text">Edit FAQ</h1>
    </div>
    ...
    <form action="<?= base_url('admin/faqs/update/' . $faq['id']) ?>" method="POST" class="grid grid-cols-1 xl:grid-cols-3 gap-8">
        <?= csrf_field() ?>
        ...
        <div class="xl:col-span-2 space-y-6">
            <div class="bg-surface rounded-2xl border border-border shadow-sm p-6 space-y-6">
                <div>
                    ...
                    <input type="text" id="question" name="question" value="<?= old('question', $faq['question']) ?>" required class="w-full px-4 py-3 bg-surface-secondary border border-border rounded-xl text-text focus:outline-none focus:ring-2 focus:ring-brand-primary">
                </div>
                ...
                <div>
                    ...
                    <textarea id="answer" name="answer" rows="6" required class="w-full px-4 py-3 bg-surface-secondary border border-border rounded-xl text-text focus:outline-none focus:ring-2 focus:ring-brand-primary"><?= esc(old('answer', $faq['answer'])) ?></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="bg-surface rounded-2xl border border-border shadow-sm p-6 space-y-6">
                ...
                <div>
                    ...
                    <input type="number" id="sort_order" name="sort_order" value="<?= old('sort_order', $faq['sort_order']) ?>" class="w-full px-4 py-2 bg-surface-secondary border border-border rounded-lg text-text focus:outline-none focus:ring-2 focus:ring-brand-primary">
            ...
            <!-- Relationships -->
            <div class="bg-surface rounded-2xl border border-border shadow-sm p-6 space-y-6">
                <h3 class="font-bold text-text border-b border-border pb-3">Contextual Display</h3>
                ...
                <div>
                    ...
                    <div class="max-h-40 overflow-y-auto bg-surface-secondary border border-border rounded-lg p-3 space-y-2">
                        <?php 
                        $selectedServices = old('services', $faq['services']);
                        foreach ($services as $srv): 
                        ?>
                            <label class="flex items-center">
                                <input type="checkbox" name="services[]" value="<?= $srv['id'] ?>" class="rounded border-border text-brand-primary focus:ring-brand-primary" <?= is_array($selectedServices) && in_array($srv['id'], $selectedServices) ? 'checked' : '' ?>>
                ...
                <div>
                    ...
                    <div class="max-h-40 overflow-y-auto bg-surface-secondary border border-border rounded-lg p-3 space-y-2">
                        <?php 
                        $selectedIndustries = old('industries', $faq['industries']);
                        foreach ($industries as $ind): 
                        ?>
                            <label class="flex items-center">
                                <input type="checkbox" name="industries[]" value="<?= $ind['id'] ?>" class="rounded border-border text-brand-primary focus:ring-brand-primary" <?= is_array($selectedIndustries) && in_array($ind['id'], $selectedIndustries) ? 'checked' : '' ?>>
